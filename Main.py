...
running = True
while running:
  NewMessages, rlist, xlist = select([server], [],[], 0.1)
  if len(NewMessages) > 0 :
    ...
    for line in recv.split("\r\n"):
      ...
      #Join
        ...
        if linex[2][1:] != "#opers":
          ...
          #It's an user that join the channel
            ...
            try:
              if authentificated.count(client):
                for channel in data["channels"]:
                  SendCommand("Mode %s %s %s" % (channel, auth[channel][sender][0], sender))
            except KeyError:
              tools.log("+" + GetNameWithoutPrefix(client))
              auth[linex[2][1:]][GetNameWithoutPrefix(client)] = []
      ...
      #Part
        ...
        authentificated.remove(GetSender(linex))
            ...
            if len(commande) == 2:
              try:
                if auth[channel][sender][1] == hashlib.sha1(commande[1].encode()).hexdigest():
                  for channel in data["channels"]:
                    SendCommand("Mode %s %s %s" % (channel, auth[channel][sender][0], sender))
                  if authentificated.count(sender) == 0:
                    authentificated.append(sender)
                else:
      ...
      #Kick
        ...
        authentificated.remove(linex[3])
