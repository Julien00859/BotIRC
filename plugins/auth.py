                    ...
                    if sha256(args[2].encode()).hexdigest() == auth[sender]["password"]:
                        ...
                        for channel in API.get_channels():
                            if sender in API.get_users(channel) and channel in auth[sender]["channels"]:
                                API.set_user_mode(channel, auth[sender]["channels"][channel], sender)
...
def on_quit(sender):
    if sender in auth and "authenticated" in auth[sender] and auth[sender]["authenticated"]:
        auth[sender]["authenticated"] = False